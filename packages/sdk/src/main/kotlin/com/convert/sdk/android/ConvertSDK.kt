...
private fun buildSharedJson(): Json = Json {
    ...
    explicitNulls = false
    // Story 3.4: the Convert backend emits rule-element payloads with
    // no class discriminator — the element's rule_type is implicit in
    // its shape. kotlinx-serialization cannot deserialise the generated
    // RuleElement* interfaces without a registered fallback. This
    // module supplies a "catch-all" deserializer that wraps each rule
    // element's raw JsonObject in a concrete holder the RuleManager
    // walks by JSON keys. Without this, any config response carrying
    // audience/location rules fails to parse and the SDK stays
    // unready forever.
    //
    // Story 4.1: combined module also registers AnyAsJsonElementSerializer
    // as the contextual serializer for Any, so generated @Contextual Any?
    // fields (notably ExperienceChangeFullStackFeatureBaseAllOfData.
    // variablesData) deserialize to JsonElement. FeatureManager depends
